/**
 * Client creates /workers and /tasks znodes; submits 10 tasks, each named /tasks/task-000000000d where d = 0-9; waits
 * until /workers and /tasks have no more children; and deletes these two znodes.
 */
...
public class Client implements Watcher, Closeable {
    ...
    /**
     * Joins ZooKeeper session at the port given through the constructor. The session will be expired at 15 seconds for
     * no communication.
     */
    public void startZK() throws IOException, InterruptedException, KeeperException {
        zk = new ZooKeeper(hostPort, 15000, this);
        if (zk.exists("/client", false) != null) {
            System.out.println("Another client is already running. Exiting...");
            System.exit(1);
        }
        zk.create("/client", new byte[0], Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);
    }
    ...
    public static void main(String args[]) throws Exception {
        if (args.length == 2)
            nTasksSubmitted = Integer.parseInt(args[1]);
        else
            nTasksSubmitted = 10;
    ...
    private static int nTasksSubmitted;
    ...
    /**
     * Creates the /tasks znode synchronously and thereafter submits /tasks/task-000000000d where d=0-9. Each task has
     * "submitted" as its data
     */
    private void createBagOfTasks() throws Exception {
        ...
        if (tasks.size() < nTasksSubmitted) {
            for (int i = tasks.size(); i < nTasksSubmitted; i++) {
    ...
    /**
     * Launches a watcher for each task, confirms all tasks have been deleted, deletes /tasks, checks all workers are
     * gone, and finally deletes /workers.
     */
    private void confirmEmptyBag() throws Exception {

        // for each task, launch a task watcher.
        for (int i = 0; i < nTasksSubmitted; i++) {
            String taskNumber = String.format("%010d", i); // This will pad the number with leading zeros to make it
            // 10 digits long
            String taskPath = "/tasks/task-" + taskNumber;
        ...
        // Waits until all workers disappeared. Then, we can delete /workers.
        while (true) {
    ...
    /**
     * Watches any changes of /task and increments nTaskCompleted if the change was a task-deleting event. Otherwise,
     * this method reschedules a task watcher for detecting future events.
     */
