/**
 * Worker keeps picking up an available or an incomplete task from a bag of tasks. Once all tasks are exhausted, it gets
 * terminated.
 */
...
public class Worker implements Watcher, Closeable {
     ...
     * <p>
     * Registers this worker under /workers znode. The worker should be identified as /workers/worker-000000000d where
     * d=0-9. It's ephemral and stored in workerID.
     */
    ...
    /**
     * This is your homework assignment. Gets a list of tasks from /tasks, checks each task-000000000d where d=0-9,
     * picks up one if its data is "submitted", otherwise examines if its timestamp (i.e., data) gets expired beyond 100
     * seconds, and if so picks it up as updating its timestamp to the present. This method returns task-000000000d as a
     * task ID to execute or "job stalled" if all remaining tasks are being executed below 100 seconds. If no more tasks
     * are found under /tasks, the method returns null.
     *
     * @return task-00000000d where d=0-9, as a taskID to execute.
     */
    private String pickupTask() {
        ...
        try {
            ...
            for (int i = 0; children != null && i < children.size(); i++) {
                ...
                String taskStatus
                        = new String(zk.getData("/tasks/" + children.get(i), false, taskStat));
     ...
     * This is your homework assignment.
     * <p>
     * Receives a taskID, (i.e., task-00000000d where d = 0-9), converts it to 500,000 - 5,0000,000 vertices, and
     * runs:<br> java -Xss512m GraphBridge vertices
     *
     * @param taskID a task obtained from the bag of tasks, from which the worker runs "java -Xss512m GraphBridge
     *               (taskID + 1) * 1000000/2
     */
